...
class TestPubNubSubscription(unittest.TestCase):
    ...
    def test_subscribe_pub_unsubscribe(self):
        ...
        try:
            ...
            pubnub.publish().channel(ch).message(message).async_(publish_operation.callback)

            if publish_operation.await_() is False:
    ...
    def test_subscribe_cg_publish_unsubscribe(self):
        ...
        pubnub.add_channel_to_channel_group() \
            .channel_group(gr) \
            .channels(ch) \
            .async_(non_subscribe_listener.callback)
        ...
        pubnub.publish().message(message).channel(ch).async_(non_subscribe_listener.callback)
        result = non_subscribe_listener.await_result_and_reset()
